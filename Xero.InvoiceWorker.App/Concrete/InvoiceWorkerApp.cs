...
namespace Xero.InvoiceWorker.App
{
    public class InvoiceWorkerApp: IInvoiceWorkerApp
    {
        ...
        public async Task Subscribe(string endPoint, string invoiceDirectory)
        {
            ...
            var eventFeed = await _service.Subscribe(endPoint + _config.InvoiceApiEndpoint);
            await _service.ProcessEventFeedCollection(eventFeed, invoiceDirectory, _config.TemplateRootPath);
        }
    }
}
